...
SELECT DEPARTMENT_NO, DEPARTMENT_NAME, STUDENT_NO, STUDENT_NAME, SUM(POINT)
FROM TB_STUDENT
JOIN TB_DEPARTMENT USING (DEPARTMENT_NO)
JOIN TB_GRADE USING (STUDENT_NO)
--WHERE DEPARTMENT_NAME = '국어국문학과'      -- HAIVNG절에서 필터링이 되면 결과가 1개만 나오기 때문에 메인에서 생략해도 괜찮음.
GROUP BY STUDENT_NO, STUDENT_NAME, DEPARTMENT_NO, DEPARTMENT_NAME
HAVING SUM(POINT) = (
    ...
    GROUP BY DEPARTMENT_NO, DEPARTMENT_NAME, STUDENT_NO, STUDENT_NAME)
ORDER BY 5 DESC
;
SELECT S.STUDENT_NO, S.STUDENT_NAME, TRUNC(AVG(POINT),1)
FROM TB_STUDENT S, TB_DEPARTMENT D, TB_GRADE G
WHERE S.DEPARTMENT_NO = D.DEPARTMENT_NO
AND S.STUDENT_NO = G.STUDENT_NO
AND D.DEPARTMENT_NAME = '국어국문학과'        -- AVG 함수로 하면 결과가 같은 값이 여럿 있어서 메인에서 조건을 지울 수 없음.
GROUP BY S.STUDENT_NO, S.STUDENT_NAME
HAVING AVG(POINT) = (
    ...
    GROUP BY S.STUDENT_NO, S.STUDENT_NAME)
;
-- Ex15-1) RANK 함수를 이용해서 학과별로 1등을 알아봅세당
SELECT * FROM (
    SELECT DEPARTMENT_NAME 학과, STUDENT_NO 학번, STUDENT_NAME 성명, TRUNC(AVG(POINT),2) 평점
            -- 그룹으로 묶어서 순위를 구한다.
            -- PARTITION BY 그룹으로 묶을 컬럼명
             , RANK() OVER (PARTITION BY DEPARTMENT_NO ORDER BY TRUNC(AVG(POINT),2) DESC) RK
    FROM TB_STUDENT
    JOIN TB_DEPARTMENT USING (DEPARTMENT_NO)
    JOIN TB_GRADE USING (STUDENT_NO)
    GROUP BY STUDENT_NO, STUDENT_NAME, DEPARTMENT_NO, DEPARTMENT_NAME )
WHERE RK = 1              
;
-- Ex15-2) ORDER BY DESC로 정렬해서 ROWNUM으로 1등 찾기
SELECT * FROM
    (SELECT S.STUDENT_NO, S.STUDENT_NAME, TRUNC(AVG(POINT),1)
    FROM TB_STUDENT S, TB_DEPARTMENT D, TB_GRADE G
    WHERE S.DEPARTMENT_NO = D.DEPARTMENT_NO
...
-- Ex16) "환경조경학과"가 속한 같은 계열 학과들의 
-- 학과 별 전공과목 평점을 파악하기 위한 적절한 SQL문을 찾아내시오.
-- 단, 출력헤더는 "계열 학과명", "전공평점"으로 표시되도록 하고, 
-- 평점은 소수점 둘째자리까지만 반올림하여 표시되도록 한다.
SELECT * FROM
    (SELECT DEPARTMENT_NAME "계열 학과명", ROUND(AVG(POINT),2) "전공평점"
    FROM TB_DEPARTMENT
    JOIN TB_CLASS USING (DEPARTMENT_NO)
    JOIN TB_GRADE USING (CLASS_NO)
...
SELECT * FROM
    (SELECT DEPARTMENT_NAME "계열 학과명", ROUND(AVG(POINT),2) "전공평점"
            , RANK() OVER (ORDER BY ROUND(AVG(POINT),2) DESC) RK
    FROM TB_DEPARTMENT
    JOIN TB_CLASS USING (DEPARTMENT_NO)
    JOIN TB_GRADE USING (CLASS_NO)
    WHERE CATEGORY = (SELECT CATEGORY 
